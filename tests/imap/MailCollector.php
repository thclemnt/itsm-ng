...
class MailCollector extends DbTestCase
{
    ...
    public function testCollect()
    {
        ...
        $this->integer(count($iterator))->isGreaterThanOrEqualTo(count($not_imported_specs));
        ...
        foreach ($not_imported_specs as $not_imported_spec) {
            $this->boolean(in_array($not_imported_spec, $not_imported_values, true))->isTrue();
        }
        ...
        foreach ($actors_specs as $actor_specs) {
            ...
            sort($names);
            $expected_names = $actor_specs['tickets_names'];
            sort($expected_names);
