...
class User extends CommonDBTM
{
    ...
    public function prepareInputForUpdate($input)
    {
        ...
        if (isset($input['picture']) && !empty($input['picture'])) {
            if (is_array($input['picture'])) {
                $decoded_picture = $input['picture'];
            } else {
                $decoded_picture = json_decode(stripslashes((string) $input['picture']), true);
            }

            if (
                is_array($decoded_picture)
                && isset($decoded_picture[0])
                && is_array($decoded_picture[0])
            ) {
                $picture = $decoded_picture[0];
                unset($input['picture']);
            }
        }
