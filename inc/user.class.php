...
class User extends CommonDBTM
{
    ...
    public function prepareInputForAdd($input)
    {
        ...
        if (!Auth::isValidLogin(stripslashes((string) $input['name']))) {
            ...
            if (empty($input["password"])) {
                ...
                if ($input["password"] == $input["password2"]) {
                    if (Config::validatePassword($input["password"])) {
                        $input["password"]
                           = Auth::getPasswordHash(Toolbox::unclean_cross_side_scripting_deep(stripslashes((string) $input["password"])));
    ...
    public function prepareInputForUpdate($input)
    {
        ...
        if (isset($input['picture']) && !empty($input['picture'])) {
            $picture = json_decode(stripslashes((string) $input['picture']), true)[0];
            unset($input['picture']);
        }
                    ...
                    if (
                        ...
                        && (($input['id'] == Session::getLoginUserID())
                            ...
                            || (($input['password_forget_token'] == $this->fields['password_forget_token'])
                                && (abs(strtotime((string) $_SESSION["glpi_currenttime"])
                                    - strtotime((string) $this->fields['password_forget_token_date'])) < DAY_TIMESTAMP)))
                    ) {
                        $input["password"]
                           = Auth::getPasswordHash(Toolbox::unclean_cross_side_scripting_deep(stripslashes((string) $input["password"])));
            ...
            if (isset($this->fields["id"]) && ($this->fields["id"] > 0)) {
                ...
                if ($ds) {
                    ...
                    if (
                        ...
                        || !file_exists($oldfile)
                        || sha1_file($oldfile) !== sha1((string) $img)
                    ) {
            ...
            if (isset($this->fields["id"]) && ($this->fields["id"] > 0)) {
                ...
                if (
                    ...
                    foreach ($this->input["_emails"] as $email) {
                        if (!in_array(strtolower((string) $email), array_map(strtolower(...), $unique_emails))) {
                            $unique_emails[] = $email;
                        }
                    }
     ...
     *
     * @return string|bool Basedn of the user / false if not found
     */
        ...
        if (count($group_fields)) {
            ...
            for ($i = 0; $i < $v['count']; $i++) {
                ...
                if (
                    ...
                    $v[$i]['ou'] = [];
                    for ($tmp = $v[$i]['dn']; count($tmptab = explode(',', (string) $tmp, 2)) == 2; $tmp = $tmptab[1]) {
                        $v[$i]['ou'][] = $tmptab[1];
                    }
                        ...
                        case "email4":
                            ...
                            if (!empty($v[0][$e])) {
                                foreach ($v[0][$e] as $km => $m) {
                                    if (!preg_match('/count/', (string) $km)) {
                                        $this->fields["_emails"][] = addslashes((string) $m);
                                    }
                                }
        ...
        foreach ($a_field as $field => $value) {
            if (
                ...
                switch ($field) {
                    ...
                    case "email4":
                        // Manage multivaluable fields
                        if (!preg_match('/count/', (string) $_SERVER[$value])) {
                            $this->fields["_emails"][] = addslashes((string) $_SERVER[$value]);
                        }
                        // Only get them once if duplicated
                        $this->fields["_emails"] = array_unique($this->fields["_emails"]);
                        break;
                    ...
                    case "title":
                        $this->fields['usertitles_id']
                              = Dropdown::importExternal('UserTitle', addslashes((string) $_SERVER[$value]));
                        break;

                    case "category":
                        $this->fields['usercategories_id']
                              = Dropdown::importExternal('UserCategory', addslashes((string) $_SERVER[$value]));
                        break;
     ...
     *
     * @return void|bool false if user is not the current user, otherwise print form
     *
     * @since 0.84
     */
    ...
    public function pre_updateInDB()
    {
        ...
        if (($key = array_search('name', $this->updates)) !== false) {
            ...
            if (!Auth::isValidLogin(stripslashes((string) $this->input['name']))) {
                $this->fields['name'] = $this->oldvalues['name'];
                unset($this->updates[$key]);
                unset($this->oldvalues['name']);
                Session::addMessageAfterRedirect(
        ...
        if (
            ...
            && !strpos((string) $_SERVER['PHP_SELF'], "/front/login.php")
            && isset($this->fields["authtype"])
        ) {
            // extauth ldap case
            if (
     ...
     * @param bool         $inactive_deleted true to retreive also inactive or deleted users
     *
    ...
    public static function dropdown($options = [])
    {
        ...
        if ((strlen((string) $p['value']) == 0) || !is_numeric($p['value'])) {
            $p['value'] = 0;
        }
     ...
     *
     * @param bool $display Whether to display or return output
     * @param array   $options Options
    ...
    public function getPasswordExpirationTime()
    {
        ...
        return strtotime(
            ...
            strtotime((string) $this->fields['password_last_update'])
        );
    }
