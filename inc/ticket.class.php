...
class Ticket extends CommonITILObject
{
           ...
           || (Session::haveRight(self::$rightname, self::READASSIGN)
              && ($this->isUser(CommonITILActor::ASSIGN, Session::getLoginUserID())
                 ...
                 || (Session::haveRight(self::$rightname, self::ASSIGN)
                    && ($this->fields["status"] == $_SESSION['INCOMING']))))
           || (Session::haveRightsOr('ticketvalidation', TicketValidation::getValidateRights())
    ...
    public function canReopen()
    {
        return Session::haveRight('followup', CREATE)
           ...
           && ($this->isAllowedStatus($this->fields['status'], $_SESSION['INCOMING'])
              || $this->isAllowedStatus($this->fields['status'], $_SESSION['ASSIGNED']));
    }
        ...
        if ($donotif && $CFG_GLPI["use_notifications"]) {
            ...
            if (
                ...
                && (isset($this->input['status'])
                  ...
                  && ($this->oldvalues['status'] == $_SESSION['SOLVED']))
            ) {
    ...
    public function prepareInputForAdd($input)
    {
        ...
        if (
            ...
            $input["status"] = $_SESSION['ASSIGNED'];
        }
    ...
    public function post_addItem()
    {
        ...
        if (
            ...
            $type = "new";
            if (isset($this->fields["status"]) && ($this->fields["status"] == $_SESSION['SOLVED'])) {
                $type = "solved";
            }
            $toadd = [
               'type'       => $type,
    ...
    public function canAddItem($type)
    {
        ...
        if ($type == 'Document') {
            if ($this->getField('status') == $_SESSION['CLOSED']) {
                return false;
            }
     ...
     * @return array
     **/
    public static function getAllStatusArray($withmetaforsearch = false, $alldata = false)
    {
    ...
    {
        return [$_SESSION['ASSIGNED'], $_SESSION['PLANNED']];
    }
    ...
    public function showFormHelpdesk($ID, $ticket_template = false)
    {
        ...
        if (
            ...
            $url_validate = Ticket::getSearchURL() . "?" . Toolbox::append_params(
                $opt,
                '&amp;'
            );
    ...
    public static function showFormHelpdeskObserver($options = [])
    {
        ...
        // override default value by function parameters
        if (is_array($options) && count($options)) {
            foreach ($options as $key => $val) {
                $params[$key] = $val;
            }
        ...
        if (isset($params['_users_id_observer_notif']['use_notification'])) {
            if (is_array($params['_users_id_observer_notif']['use_notification'])) {
                $use_notif = $params['_users_id_observer_notif']['use_notification'][$user_index] ?? true;
            } else {
        ...
        if ($user_index == 0) {
            ...
            echo "function addNewObserver(rand) {";
            echo "  var newIndex = $('.observer-container').length;";
            ...
            echo "  newObserver += '</div>';";
            echo "  $('#addObserver' + rand).before(newObserver);";
            echo "}";
            echo "</script>";
        }

        if ($user_index == 0) {
            echo "<div id='observer_container'></div>";
        }
    }
     ...
     **/
    public function getTicketTemplateToUse(
        $force_template = 0,
        $type = 0,
        $itilcategories_id = 0,
        $entities_id = -1
    ...
    public function generateFollowupLink($actor, $actorType)
    {
        global $CFG_GLPI;
    ...
    private function normalizeTicketActorInputValues($value)
    {
        if ($value === null || $value === '') {
            return [];
        }
    ...
    private function getTicketActorFieldDefinitions($action)
    {
        switch ($action) {
    ...
    private function getTicketActorDisplayName($itemtype, $id, $fallback = '')
    {
        if ((int)$id <= 0) {
            return $fallback;
        }
    ...
    private function getTicketActorPanelTitle($baseTitle, $tt, array $fields)
    {
    ...
    private function getTicketActorPanels($ID, array $options, $tt)
    {
        $isNew = $this->isNewID($ID);
        $hiddenFields = $this->getTicketActorTemplateHiddenFields($tt);
        ...
        $roleDefinitions = [
            [
                ...
                'actorType'     => 'requester',
                'title'         => $this->getTicketActorPanelTitle(_n('Requester', 'Requesters', 1), $tt, [
                    '_users_id_requester',
                    '_groups_id_requester',
                ]),
            ...
            [
                ...
                'actorType'     => 'observer',
                'title'         => $this->getTicketActorPanelTitle(_n('Watcher', 'Watchers', Session::getPluralNumber()), $tt, [
                    '_users_id_observer',
                    '_groups_id_observer',
                ]),
            ...
            [
                ...
                'actorType'     => 'assign',
                'title'         => $this->getTicketActorPanelTitle(__('Assigned to'), $tt, [
                    '_users_id_assign',
                    '_groups_id_assign',
                    '_suppliers_id_assign',
                ]),
        ...
        foreach ($roleDefinitions as $panelDefinition) {
            $definitions = $this->getTicketActorFieldDefinitions($panelDefinition['action']);
            $actorTypes = [];
    ...
    private function renderTicketActorPanels($ID, array $options, $tt)
    {
        global $CFG_GLPI;
     ...
     **/
    public static function showDocumentAddButton($size = 25)
    {
        ...
        switch ($status) {
            case "waiting": // waiting tickets
                $WHERE = array_merge(
                    $WHERE,
                    $search_assign,
                    ['glpi_tickets.status' => $_SESSION['WAITING']]
                );
                break;
            ...
            case "process": // planned or assigned or incoming tickets
                $WHERE = array_merge(
                    $WHERE,
                    $search_assign,
                    ['glpi_tickets.status' => array_merge(self::getProcessStatusArray(), [$_SESSION['INCOMING']])]
                );
            ...
            case "tovalidate": // tickets waiting for validation
                ...
                $WHERE = array_merge(
                    ...
                    [
                      ...
                      'NOT'                            => [
                         'glpi_tickets.status'   => [$_SESSION['SOLVED'], $_SESSION['CLOSED']]
                      ]
                    ]
                );
                break;
            ...
            if ($showgrouptickets) {
                switch ($status) {
                    case "toapprove":
                        ...
                        $options['criteria'][0]['value']      = $_SESSION['SOLVED'];
                    ...
                    case "waiting":
                        $options['criteria'][0]['field']      = 12; // status
                        $options['criteria'][0]['searchtype'] = 'equals';
                        $options['criteria'][0]['value']      = $_SESSION['WAITING'];
                        $options['criteria'][0]['link']       = 'AND';
                ...
                switch ($status) {
                    ...
                    case "toapprove":
                        ...
                        $options['criteria'][3]['searchtype'] = 'equals';
                        $options['criteria'][3]['value']      = $_SESSION['SOLVED'];
                        $options['criteria'][3]['link']       = 'AND';
                    ...
                    case "survey":
                        $options['criteria'][0]['field']      = 12; // status
                        $options['criteria'][0]['searchtype'] = 'equals';
                        $options['criteria'][0]['value']      = $_SESSION['CLOSED'];
                        $options['criteria'][0]['link']       = 'AND';
     ...
     **/
    public static function getSplittedSubmitButtonHtml($tickets_id, $action = "add")
    {
